...
| **`aigovops` — Open Source v4** | `aigovops-foundation/aigovops` | Self-hostable governance stack from a clone: zero-dependency Node gate (`up --tier 1` = just the gate), Caddy/Keycloak/OpenSearch/Prometheus at tier 4; 193 tests; npm packages and `get.aigovops.org` planned, not live | 200, 300 | Pre-pend; Operate | Get; Stay | The tier-1 gate is the "gate one real call today" path in the 200 worksheet until the Beacon Python SDK lands; npm publish is the August plan's wedge |
| **aigovops-Replay** | `aigovops-foundation/aigovops-Replay` | Cryptographically signed receipt system for AI interactions | — | Audit | Recover | Fourth receipt implementation in the estate (the August plan counted four); folds onto the one OVERT envelope or is archived in the copy sweep |
| **aigovops-prompt-studio** | `aigovops-foundation/aigovops-prompt-studio` | Prompt engineering platform with immutable audit logging and version control | — | Policy gates | Get | Its audit log adopts the receipt; otherwise out of the ladder |
